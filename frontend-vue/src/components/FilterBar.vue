<template>
  <div class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 transition-colors duration-300 space-y-4">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-100 pb-4 dark:border-slate-800">
      <div class="flex items-center gap-2">
        <SlidersHorizontal class="h-5 w-5 text-amber-500" />
        <h3 class="text-base font-bold text-slate-900 dark:text-white">Filter & Search Venues</h3>
      </div>
      <button
        type="button"
        @click="clearFilters"
        class="inline-flex items-center gap-1.5 text-xs font-bold text-slate-500 hover:text-amber-600 dark:hover:text-amber-400 transition-colors"
      >
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
      ...
      <div class="space-y-1">
        ...
        <div class="relative">
          <input
            ...
            @input="emitFilters"
            placeholder="e.g. Russian River, Santa Rosa..."
            class="w-full rounded-xl border border-slate-200 bg-slate-50 pl-10 pr-4 py-2 text-sm dark:border-slate-800 dark:bg-slate-950 focus:border-amber-500 focus:outline-none dark:text-white"
          />
      ...
      <div class="space-y-1">
        ...
        <select
          ...
          @change="emitFilters"
          class="w-full rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm dark:border-slate-800 dark:bg-slate-950 focus:border-amber-500 focus:outline-none dark:text-white"
        >
      ...
      <div class="space-y-1">
        ...
        <div class="flex items-center gap-3 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl px-4 py-2 text-sm">
          <input
            ...
            step="1"
            @input="emitFilters"
            class="w-full accent-amber-500 bg-slate-200 dark:bg-slate-800 rounded-lg cursor-pointer"
          />
        ...
        <button
          ...
          :class="[
            ...
            filters.amenities.includes(amenity.slug)
              ? 'bg-amber-500 border-amber-500 text-white shadow-sm dark:bg-amber-600 dark:border-amber-600'
