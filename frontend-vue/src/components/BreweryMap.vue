<template>
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="lg:col-span-1 rounded-xl border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900 transition-colors duration-300 flex flex-col h-[500px]">
    ...
    <div class="lg:col-span-2 flex flex-col gap-6">
      <!-- Location & Visual Map Mockup -->
      <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900 transition-colors duration-300 flex-1">
        <div v-if="selectedBrewery" class="space-y-6">
          <div class="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
            <div>
              <span class="inline-flex items-center rounded-full bg-amber-50 px-2 py-0.5 text-xs font-semibold text-amber-800 dark:bg-amber-950/30 dark:text-amber-400 border border-amber-100 dark:border-amber-900/60 mb-2">
            ...
            <div class="relative w-full sm:w-48 h-32 rounded-lg bg-slate-100 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 overflow-hidden flex flex-col items-center justify-center p-4">
              <!-- Grid Background Mock -->
              <div class="absolute inset-0 bg-[radial-gradient(#e2e8f0_1px,transparent_1px)] dark:bg-[radial-gradient(#1e293b_1px,transparent_1px)] [background-size:12px_12px] opacity-70"></div>
              
              <!-- Location Pointer -->
              <div class="relative flex flex-col items-center z-10 animate-bounce">
                <MapPin class="h-7 w-7 text-rose-500 fill-rose-500/20" />
                <div class="absolute top-1 h-2 w-2 rounded-full bg-rose-500"></div>
              </div>
              
              <span class="relative z-10 text-[10px] font-mono text-slate-500 dark:text-slate-400 bg-white/80 dark:bg-slate-900/80 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-800 mt-1">
